...
import { ShieldCheck, Search, Filter, Download, Plus, ChevronRight, MoreHorizontal, X, FileUp, Edit, Trash2 } from 'lucide-react';

const DBSVetting = () => {
  const [searchQuery, setSearchQuery] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [formData, setFormData] = useState({ "employee": "", "dbsNumber": "", "issueDate": "", "renewalDate": "", "document": "", status: 'Active' });
  ...
  const handleAddRecord = () => {
    ...
    setFormData({ "employee": "", "dbsNumber": "", "issueDate": "", "renewalDate": "", "document": "", status: 'Active' });
  };
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between rounded-3xl bg-gradient-to-r from-brand-800 to-brand-600 p-6 md:p-8 text-white shadow-lg shadow-brand-900/10">
        <div>
          ...
          <p className="mt-1 text-sm md:text-base text-brand-100 font-medium">
            Pre-employment checks, Right to Work, and DBS renewals.
          </p>
        </div>
