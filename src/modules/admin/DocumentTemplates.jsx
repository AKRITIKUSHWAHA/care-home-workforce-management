...
const AdminDocumentTemplates = () => {
  ...
  const handleGenerateSubmit = (e) => {
    ...
    // Save in context
    saveGeneratedDocument(employee.id, docName, fileName, generatedHtml, "Admin");
    ...
    setTimeout(() => {
      ...
      container.id = 'temp-pdf-generation-holder';
      container.style.position = 'absolute';
      container.style.left = '-9999px';
      container.innerHTML = generatedHtml;
      document.body.appendChild(container);
      triggerPDFDownload('temp-pdf-generation-holder', docName).then(() => {
  ...
  generatedHistory.sort((a, b) => {
    // Parse time / date strings (dd/mm/yyyy)
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="h-6.5 w-6.5 rounded-lg flex items-center justify-center bg-red-50 hover:bg-red-100 text-red-600 dark:bg-red-950/20 dark:hover:bg-red-950/40 transition-colors"
                  ...
                  return (
                    <div id="live-generator-preview-holder" className="p-8 letter-preview relative text-sm leading-relaxed min-h-[550px] font-sans">
                      ...
                      <div className="mb-6 text-xs space-y-0.5">
                        <strong className="text-slate-500 block mb-0.5">Recipient Details:</strong>
                      ...
                      <div className="whitespace-pre-wrap text-slate-600 dark:text-slate-350 font-normal leading-relaxed mb-10 text-xs">
                  ...
                  {filteredHistory.length === 0 ? (
                    ...
                    filteredHistory.map((h, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/45 dark:hover:bg-slate-900/10 transition-colors">
                        ...
                        <td className="p-3.5">
                          <span className="px-2 py-0.5 rounded font-extrabold text-[8px] border uppercase bg-indigo-50 text-indigo-700 border-indigo-250 dark:bg-indigo-500/10 dark:text-indigo-400">
                            {h.uploadedBy}
                          </span>
                        </td>
                        <td className="p-3.5 text-right pr-5">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              onClick={() => {
                                // Direct render preview modal using object content
                                const pseudoTemplate = {
                                  name: h.name,
                                  subject: h.name,
                                  content: '', // not needed, HTML exists
                                  includeLogo: false, // not needed, HTML exists
                                  requireManagerSignature: false, // not needed, HTML exists
                                  requireEmployeeSignature: false // not needed, HTML exists
                                };
                              ...
                              className="h-7 px-3 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-750 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 transition-colors text-[9px] font-bold flex items-center gap-1"
                            >
                              <Eye className="h-3.5 w-3.5" />
                            ...
                            <button
                              onClick={() => {
                                const container = document.createElement('div');
                                container.id = 'temp-pdf-history-holder';
                                container.style.position = 'absolute';
                                container.style.left = '-9999px';
                                container.innerHTML = h.contentHtml;
                                document.body.appendChild(container);
                                triggerPDFDownload('temp-pdf-history-holder', h.name).then(() => {
                                  document.body.removeChild(container);
                                });
                              }}
          ...
          <div className="glass-modal max-w-3xl w-full rounded-3xl p-5 border border-slate-200 dark:border-slate-800 shadow-2xl relative bg-white dark:bg-slate-950 animate-slide-up max-h-[calc(100vh-2rem)] flex flex-col justify-start">
            ...
            <div className="flex justify-between items-center border-b pb-3 mb-4 dark:border-slate-800">
              ...
              <div className="flex items-center gap-2 pr-6">
                {/* PDF Download Button from preview */}
                <button
                  onClick={() => {
                    triggerPDFDownload('modal-document-preview-holder', previewTemplate.name);
                  }}
                ...
                {/* Employee dropdown selector (only when NOT showing a direct generated HTML file) */}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-1.5 border border-slate-100 dark:border-slate-800/80 rounded-2xl bg-slate-100/60 dark:bg-slate-900/10">
              <div className="bg-white text-slate-800 shadow-md rounded-xl max-w-[800px] mx-auto overflow-hidden">
                ...
                {previewEmployee.__directHtml ? (
                  /* RENDER GENERATED LETTER HTML */
                  <div 
                    id="modal-document-preview-holder"
                    dangerouslySetInnerHTML={{ __directHtml: undefined, __html: previewEmployee.__directHtml }} 
                  />
                ) : (
                  /* RENDER DYNAMIC TEMPLATE PREVIEW */
                  <div id="modal-document-preview-holder" className="p-8 letter-preview relative text-xs leading-relaxed font-sans">
                    {/* Header */}
