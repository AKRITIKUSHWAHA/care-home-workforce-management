...
import { FileWarning, Search, Filter, Download, Plus, ChevronRight, MoreHorizontal, X, FileUp, Edit, Trash2 } from 'lucide-react';

const Complaints = () => {
  const [searchQuery, setSearchQuery] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [formData, setFormData] = useState({ "complaintId": "", "receivedDate": "", "complainant": "", "category": "", "details": "", status: 'Active' });
  ...
  const handleAddRecord = () => {
    ...
    setFormData({ "complaintId": "", "receivedDate": "", "complainant": "", "category": "", "details": "", status: 'Active' });
  };
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between rounded-3xl bg-gradient-to-r from-brand-800 to-brand-600 p-6 md:p-8 text-white shadow-lg shadow-brand-900/10">
        <div>
          ...
          <p className="mt-1 text-sm md:text-base text-brand-100 font-medium">
            Manage and respond to formal complaints (Duty of Candour).
          </p>
        </div>
