...
const WeeklyMedicationAudit = ({ selectedAudit, submitAuditResult, setSelectedAudit, isEditMode }) => {
  ...
  useEffect(() => {
    if (selectedAudit.status === 'Completed') {
      if (selectedAudit.details) {
        ...
        // Total questions = 22 * 5 = 110. Yes count = Math.round((110 * scoreVal) / 100);
        const yesCount = Math.round((110 * scoreVal) / 100);
        ...
        for (let s = 0; s < 5; s++) {
          weeklyMedicationConfig.questions.forEach(q => {
            if (currentYesCount < yesCount) {
              ...
              tempComments[`s${s}_${q.id}`] = 'Weekly record verification gap.';
            }
          });
        }
        ...
        for (let s = 0; s < 5 && addedCount < noCount; s++) {
          weeklyMedicationConfig.questions.forEach(q => {
            if (tempAnswers[`s${s}_${q.id}`] === 'NO' && addedCount < noCount) {
              mockActions.push({
                section: q.section,
                problem: `Sample ${s+1} (${defaultResidents[s]?.name}): Weekly gap: ${q.question.substring(0, 40)}...`,
                actions: `Rectify and verify weekly medication check for: ${q.question.substring(0, 30)}`,
  ...
  // Dynamic calculations based on formula: YES and N/A = 1, NO = 0. Divide by (22 * 5 = 110).
  const getCalculatedScore = () => {
    ...
    return Math.round((scoreCount / 110) * 100);
  };
