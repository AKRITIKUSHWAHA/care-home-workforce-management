...
import { Cross, Search, Filter, Download, Plus, ChevronRight, MoreHorizontal, X, FileUp, Edit, Trash2 } from 'lucide-react';

const EndOfLife = () => {
  const [searchQuery, setSearchQuery] = useState('');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [formData, setFormData] = useState({ "client": "", "dnacpr": "", "respectForm": "", "passport": "", "lastReviewed": "", status: 'Active' });
  ...
  const handleAddRecord = () => {
    ...
    setFormData({ "client": "", "dnacpr": "", "respectForm": "", "passport": "", "lastReviewed": "", status: 'Active' });
  };
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between rounded-3xl bg-gradient-to-r from-brand-800 to-brand-600 p-6 md:p-8 text-white shadow-lg shadow-brand-900/10">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight font-sans flex items-center gap-3">
            ...
            End of Life & Passports
          </h1>
          <p className="mt-1 text-sm md:text-base text-brand-100 font-medium">
            ReSPECT forms, DNACPR, and Hospital Care Passports.
          </p>
        </div>
